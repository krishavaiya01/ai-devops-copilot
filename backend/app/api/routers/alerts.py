...
def get_alerts(db: Session = Depends(get_db), current_user: User = Depends(require_viewer)):
    ...
    try:
        ...
        for alert_data in alerts:
            ...
            message = annotations.get(
                "description", annotations.get("summary", "Metric exceeded threshold")
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Webhook parsing failed: {str(e)}"
        )
