...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        metrics.REQUEST_LATENCY.labels(method=request.method, endpoint=path).observe(process_time)
    ...
    try:
        ...
        if db.query(User).count() == 0:
            ...
            res_1 = CloudResource(
                ...
                state="idle",
                last_scanned_at=settings.DATABASE_URL.startswith("sqlite")
                and None
                or None,  # Set dynamically
            )
            res_2 = CloudResource(
                provider="aws",
                resource_type="ebs",
